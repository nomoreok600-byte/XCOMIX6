<?php
/**
 * X COMIX - MASTER ROUTER & MANHWABUDDY ENGINE (Clean Fast Load)
 */

$current_post_id = get_the_ID();

// =========================================================================
// THE MASTER ROUTER: Check the source meta and route to the correct engine
// =========================================================================
...
// ... (Rest of your original ManhwaBuddy logic)


// =========================================================================
// MANHWABUDDY ENGINE STARTS HERE
// =========================================================================
    ...
    function xcomix_chapter_chat_render($chat, $is_logged_in, $current_user) {
        ...
        if ($chat->comment_parent > 0) {
            ...
            if ($parent_comment) {
                ...
                $reply_html = '
                <div class="flex items-center gap-1.5 text-[11px] text-gray-500 mb-1 pl-2 border-l-2 border-[#333] select-none">
                    <svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/></svg>
                    <span class="truncate">Replying to <b class="text-gray-400">@'.$p_author.'</b>: '.$p_text.'</span>
                </div>';
            }
        }
...
// 24-HOUR DATABASE CACHE
$cache_key = 'xcomix_buddy_imgs_' . $current_post_id;
...
if (false === $images) {
    ...
    if ($buddy_url) {
        ...
        if (!$html || trim($html) === '') {
            ...
            $raw_images = [];
            if (preg_match('/<div class="reading-content">(.*?)<\/div>/is', $html, $content_block)) {
                preg_match_all('/<img[^>]+(?:src|data-src|data-lazy-src)=["\']([^"\']+)["\']/i', $content_block[1], $matches);
                if (!empty($matches[1])) $raw_images = $matches[1];
            }
            if (empty($raw_images)) {
                preg_match_all('/<img[^>]+(?:src|data-src|data-lazy-src)=["\']([^"\']+)["\'][^>]*class=["\'][^"\']*wp-manga-chapter-img[^"\']*["\']/i', $html, $matches);
                if (!empty($matches[1])) $raw_images = $matches[1];
            }

            if (empty($raw_images)) {
                $diagnostic_error = "HTML loaded successfully, but 0 Buddy images found.";
            } else {
                foreach ($raw_images as $imgUrl) {
                    $cleanUrl = trim($imgUrl);
                    if (!empty($cleanUrl)) $images[] = $cleanUrl;
                }
                $images = array_values(array_unique($images));
                if (!empty($images)) set_transient($cache_key, $images, 24 * HOUR_IN_SECONDS);
            }
        }
    } else {
        $diagnostic_error = "No Buddy URL found for this chapter.";
    }
}
?>
    ...
    <main class="min-h-screen relative w-full bg-[#09090b]">
        ...
        <style>
            ...
            .orange-spinner { width: 36px; height: 36px; border: 3px solid #222; border-top-color: #ea580c; border-radius: 50%; animation: xcomix-spin 1s linear infinite; }
            ...
            #pages-container.mode-page { gap: 40px; padding-top: 80px; padding-bottom: 40px; background-color: #09090b; display: flex; flex-direction: column; align-items: center; }
        ...
        <div id="pages-container" class="mode-webtoon w-full" onclick="window.toggleReaderUI()">
            <?php if (empty($images)) { ?>
                <div class="py-32 text-center flex flex-col items-center justify-center w-full px-6">
                    <div class="w-16 h-16 bg-[#121212] rounded-full flex items-center justify-center mb-4 border border-[#333]">
                    ...
                    <span class="text-white font-black uppercase tracking-widest mb-1 text-[13px]">Scan Missing</span>
                    <span class="text-gray-500 text-[11px] font-bold tracking-wider text-center max-w-md break-words">
                        <?php echo $diagnostic_error; ?>
                        <br><br>
                        <a href="?clear_cache=1" class="text-[#3b82f6] underline">Force Re-Scan (Clear Cache)</a>
                    </span>
                </div>
            <?php } else { ?>
                ...
                ?>
                    <div class="page-wrapper page-tracker is-loading">
                        ...
                        <img data-src="<?php echo esc_url($proxied_image_url); ?>" data-raw-src="<?php echo esc_url($img); ?>" class="chapter-page" decoding="async">
                    </div>
                <?php } ?>
            <?php } ?>
        </div>
        ...
        <div id="custom-modal-overlay" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-[99999] hidden flex items-center justify-center p-4 opacity-0 transition-opacity duration-300">
        ...
        <script>
            // --- INSTANT NAVIGATION FIX: Kill pending connections to un-freeze links! ---
            ...
            // --- FAST BATCH LOADER (Max Speed, No Fake Delays) ---
            window.loadFastImages = function() {
                const images = Array.from(document.querySelectorAll('img.chapter-page[data-src]'));
                if (images.length === 0) return;

                let currentIndex = 0;
                let activeLoads = 0;
                const maxConcurrent = 3;  // Pulls 3 images at a time for fast loading
                ...
                function loadNext() {
                    while (activeLoads < maxConcurrent && currentIndex < images.length) {
                        ...
                        let spinnerContainer = img.previousElementSibling;
            ...
            window.logReadingHistory = function() {
                let history = JSON.parse(localStorage.getItem('xcomix_history') || '{}');
                history['<?php echo $manga_id; ?>'] = {
                    id: '<?php echo $manga_id; ?>',
                    title: '<?php echo esc_js($manga_title); ?>',
                    chapter: '<?php echo $chapter_num; ?>',
                    url: '<?php echo get_permalink($current_post_id); ?>',
                    timestamp: Date.now()
                };
                ...
                if (window.xcomixApp) {
                    fetch(window.xcomixApp.ajaxUrl, {
                        ...
                        body: `action=xcomix_log_history&manga_id=<?php echo $manga_id; ?>&chapter_id=<?php echo $current_post_id; ?>&chapter_num=<?php echo $chapter_num; ?>&nonce=${window.xcomixApp.nonce}`
                    });
                }
            };
            ...
            if (chatFeed && jumpBtn) {
                chatFeed.addEventListener('scroll', () => {
                    if (Math.abs(chatFeed.scrollTop) > 300) {
                        jumpBtn.classList.remove('hidden'); jumpBtn.classList.add('flex');
                    } else {
                        jumpBtn.classList.add('hidden'); jumpBtn.classList.remove('flex');
                    }
                });
            }
            ...
            window.fetchLiveChats = async function() {
                ...
                try {
                    ...
                    if(isScrolledBottom) window.scrollChatToBottom();
                        ...
                        // SMART CHAT POLLER: Only checks for messages if the user is actively looking at the page
            setInterval(() => {
                // Check if the browser tab is actually visible before pinging the server
                if (document.visibilityState === 'visible') {
                    window.fetchLiveChats();
                }
            }, 30000); // 30 seconds is much safer for your server than 15!
